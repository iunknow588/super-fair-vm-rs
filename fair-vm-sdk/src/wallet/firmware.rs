...
use k256::{
    ecdsa::{signature::hazmat::PrehashSigner, Signature as K256Signature, SigningKey},
    SecretKey,
};
use rand;
use semver::Version;
...
impl FirmwareRequirement {
    ...
    pub fn new(
        ...
        let recommended_version = Version::parse(recommended_version)?;
    ...
    pub fn check_version(&self, version: &str) -> Result<bool, FirmwareError> {
        ...
        // 检查是否在不兼容版本列表中
        if self.incompatible_versions.iter().any(|v| v == &version) {
            return Err(FirmwareError::IncompatibleVersion(version.to_string()));
        }

        // 检查是否满足最低版本要求
        if version < self.min_version {
            return Err(FirmwareError::VersionTooLow {
                current: version,
...
impl LedgerFirmwareTrait for LedgerFirmware {
    ...
    async fn sign_message(&self, message: &[u8]) -> Result<Signature, FirmwareError> {
        ...
        let r = signature_bytes[..32].into();
        let s = signature_bytes[32..].into();
        let v = 27; // 使用固定的 v 值
        Ok(Signature { r, s, v })
    }
...
mod tests {
    ...
    fn test_firmware_requirement() {
        let requirement = FirmwareRequirement::new("2.0.0", "2.0.5", &["2.1.0"]).unwrap();

        // 测试不兼容版本
        assert!(requirement.check_version("1.9.0").is_err());
        assert!(requirement.check_version("1.8.0").is_err());

        // 测试最低要求版本
        assert!(requirement.check_version("2.0.0").is_ok());

        // 测试推荐版本
        assert!(requirement.check_version("2.0.5").is_ok());

        // 测试不兼容版本
        assert!(requirement.check_version("2.1.0").is_err());
    }
